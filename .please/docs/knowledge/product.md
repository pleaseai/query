...
## Vision
A universal hybrid search engine that works with any AI model provider. Built on qmd's proven BM25 + vector + reranking pipeline, extended with a plugin-based architecture for embedding, reranking, and generation across local and cloud models.

## Problem
Existing on-device search solutions (including qmd) are tightly coupled to specific local model runtimes (node-llama-cpp / GGUF). This limits adoption by teams who prefer cloud AI providers, need multilingual models, or want to mix local and cloud models for cost/quality tradeoffs.
...
## Core Features
1. **Hybrid Search Pipeline** (from qmd): BM25 full-text + vector semantic + LLM reranking with RRF fusion
